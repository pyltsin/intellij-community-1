...
public class IncorrectDateTimeFormatInspection extends AbstractBaseJavaLocalInspectionTool {
  ...
  Map<Character, CountVerifier> DATE_TIME_FORMATTER_ALLOWED = Map.ofEntries(
    ...
    Map.entry('u', rangeOf(1, Integer.MAX_VALUE)),
    ...
    Map.entry('g', rangeOf(1, Integer.MAX_VALUE)),
    ...
    Map.entry('s', rangeOf(1, Integer.MAX_VALUE)),
    Map.entry('S', rangeOf(1, Integer.MAX_VALUE)),
    Map.entry('A', rangeOf(1, Integer.MAX_VALUE)),
    Map.entry('n', rangeOf(1, Integer.MAX_VALUE)),
    Map.entry('N', rangeOf(1, Integer.MAX_VALUE)),
  ...
  public PsiElementVisitor buildVisitor(@NotNull ProblemsHolder holder, boolean isOnTheFly) {
    return new JavaElementVisitor() {
      ...
      private void processExpression(@NotNull PsiExpression expression) {
        ...
        for (int pos = 0; pos < array.length; pos++) {
          ...
          if (!inQuote && isLetter(c)) {
            ...
            tokens.add(new Token(pos, array, length));
            checkPadding(expression, array, pos);
          ...
          checkUnsupportedProblem(expression, pos, c);
      ...
      private void checkUnsupportedProblem(@NotNull PsiExpression expression, int pos, char c) {
        if (c == '{' || c == '}' || c == '#') {
      ...
      private void checkPadding(@NotNull PsiExpression expression, char[] array, int pos) {
        char c = array[pos];
        if (c == 'p') {
          int posP = pos;
          while (posP + 1 < array.length && array[posP + 1] == 'p') {
            posP++;
          }
          int afterP = posP + 1;
          if (afterP >= array.length || !isLetter(array[afterP])) {
            TextRange range = ExpressionUtils.findStringLiteralRange(expression, pos, afterP);
            holder.registerProblem(expression, range,
                                   InspectionGadgetsBundle.message("inspection.incorrect.date.format.message.padding", c),
                                   LocalQuickFix.EMPTY_ARRAY);
          }
        }
      }
