    ...
    private static boolean isInTestContainer(PsiExpression call) {
      ...
      if (psiElement instanceof PsiLambdaExpression lambdaContainer) {
        ...
        if (!LambdaUtil.isFunctionalType(psiAnonymousClass.getBaseClassType()) ||
            InheritanceUtil.isInheritor(psiAnonymousClass, JAVA_UTIL_FUNCTION_SUPPLIER)) {
          return false;
        }
        if (!(psiAnonymousClass.getParent() instanceof PsiNewExpression psiNewExpression)) {
          return false;
        }
      ...
      if (!(expressionList instanceof PsiExpressionList psiExpressionList) ||
          !(psiExpressionList.getParent() instanceof PsiMethodCallExpression methodCallExpression)) {
        return false;
      }
