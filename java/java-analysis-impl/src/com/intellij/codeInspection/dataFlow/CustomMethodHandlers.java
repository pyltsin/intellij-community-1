...
public final class CustomMethodHandlers {
  ...
  private static DfType javaTimeLocalPlusMinus(DfaCallArguments arguments, DfaMemoryState state, PsiMethod method) {
    ...
    if (!(temporalUnitObject instanceof PsiEnumConstant)
        || ((PsiEnumConstant)temporalUnitObject).getContainingClass() == null
        || "java.time.temporal.ChronoUnit".equals(((PsiEnumConstant)temporalUnitObject).getContainingClass().getQualifiedName())) {
      return DfType.TOP;
    }
    String temporalUnitName = ((PsiEnumConstant)temporalUnitObject).getName();
    ...
    if (qualifier instanceof LocalDate) {
      if (EnumSet.of(ChronoUnit.DAYS, ChronoUnit.WEEKS, ChronoUnit.MONTHS, ChronoUnit.YEARS,
                     ChronoUnit.DECADES, ChronoUnit.CENTURIES, ChronoUnit.MILLENNIA, ChronoUnit.ERAS).contains(temporalUnit)) {
        return referenceConstant(((LocalDate)qualifier).plus(value * sign, temporalUnit), returnType);
      }
      else {
    ...
    if (qualifier instanceof LocalTime) {
      if (EnumSet.of(ChronoUnit.NANOS, ChronoUnit.MICROS, ChronoUnit.MILLIS, ChronoUnit.SECONDS, ChronoUnit.MINUTES,
                     ChronoUnit.HOURS, ChronoUnit.HALF_DAYS).contains(temporalUnit)) {
        return referenceConstant(((LocalTime)qualifier).plus(value * sign, temporalUnit), returnType);
      }
      else {
        return DfType.BOTTOM;
      }
    ...
    if (EnumSet.of(ChronoUnit.DAYS, ChronoUnit.WEEKS, ChronoUnit.MONTHS, ChronoUnit.YEARS,
                   ChronoUnit.DECADES, ChronoUnit.CENTURIES, ChronoUnit.MILLENNIA, ChronoUnit.ERAS,
                   ChronoUnit.NANOS, ChronoUnit.MICROS, ChronoUnit.MILLIS, ChronoUnit.SECONDS, ChronoUnit.MINUTES,
                   ChronoUnit.HOURS, ChronoUnit.HALF_DAYS).contains(temporalUnit)) {
      return referenceConstant(((LocalDateTime)qualifier).plus(value * sign, temporalUnit), returnType);
    }
    else {
      return DfType.BOTTOM;
    }
    ...
    for (int i = 0; i < myArguments.length; i++) {
      ...
      if (argumentValue instanceof Integer) {
        args.add((Integer)argumentValue);
               ...
               && argumentValue instanceof PsiEnumConstant
               && ((PsiEnumConstant)argumentValue).getContainingClass() != null
               && "java.time.Month".equals(((PsiEnumConstant)argumentValue).getContainingClass().getQualifiedName())
               && (returnType.equalsToText(JAVA_TIME_LOCAL_DATE_TIME) || returnType.equalsToText(JAVA_TIME_LOCAL_DATE))) {
        PsiEnumConstant enumConstant = (PsiEnumConstant)argumentValue;
  ...
  private static @NotNull DfType calendarGet(DfaCallArguments arguments, DfaMemoryState state, DfaValueFactory factory) {
    ...
    switch (val) {
      ...
      case Calendar.AM_PM:
        range = LongRangeSet.range(0, 1);
        break;
      case Calendar.DAY_OF_YEAR:
        range = LongRangeSet.range(1, 366);
        break;
      case Calendar.HOUR:
        range = LongRangeSet.range(0, 11);
        break;
      case Calendar.HOUR_OF_DAY:
        range = LongRangeSet.range(0, 23);
        break;
      case Calendar.MINUTE:
      case Calendar.SECOND:
        range = LongRangeSet.range(0, 59);
        break;
      case Calendar.MILLISECOND:
        range = LongRangeSet.range(0, 999);
        break;
    }
  ...
  private static @NotNull DfType objectGetClass(DfaCallArguments arguments,
  ...
  private static @NotNull DfType randomNextInt(DfaCallArguments arguments,
                                               DfaMemoryState state,
                                               DfaValueFactory factory,
                                               PsiMethod method) {
  ...
  private static @NotNull DfaValue collectionToArray(DfaCallArguments arguments,
                                                     DfaMemoryState state,
                                                     DfaValueFactory factory,
                                                     PsiMethod method) {
